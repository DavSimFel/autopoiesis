...
def _get_tiktoken_encoder(model_name: str) -> object | None:
    ...
    try:
        ...
        _log.debug("No exact tiktoken encoder for model %s; using fallback", model_name)
    try:
    ...
    if model_name:
        ...
        if enc is not None:
            try:
                ...
                _log.debug(
                    "Token encoding failed for model %s; falling back to char ratio",
                    model_name,
                    exc_info=True,
                )
...
def estimate_tokens_for_model(text: str, model_name: str) -> int:
    ...
    if enc is not None:
        try:
            ...
            _log.debug(
                "Token encoding failed for model %s; using model ratio fallback",
                model_name,
                exc_info=True,
            )
