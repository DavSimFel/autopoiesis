...
## Persistent Tool & Shell Output Storage

Tool call results and shell outputs are persisted to disk under the agent's
`tmp/` directory for post-hoc inspection and agent self-referencing.
...
├── tool-results/
│   └── YYYY-MM-DD/
│       └── {tool_name}_{sha256[:8]}.out   # JSON header + full content
└── shell/
    └── YYYY-MM-DD/
        └── {cmd_sha256[:8]}.log           # JSON header + stdout + stderr
```
...
```toml
[agents.default]
tmp_retention_days = 14   # age-based eviction; default 14
tmp_max_size_mb    = 500  # size-based eviction; default 500
```

Both values map to `AgentConfig.tmp_retention_days` and
`AgentConfig.tmp_max_size_mb`, and propagate to `Runtime` fields of the same name.

### Cleanup

`rotate_results(tmp_dir, retention_days, max_size_mb)` runs after every agent
turn (wired in `worker.py`).  It performs two passes:

1. **Age**: delete all `YYYY-MM-DD` directories older than `today - retention_days`
   (across both `tool-results/` and `shell/` subtrees uniformly).
2. **Size**: if total remaining size exceeds `max_size_mb`, delete the oldest
   date-directories first until within budget.

### Truncation integration

When `truncate_tool_results()` trims a tool result, it first calls
`store_tool_result()` to persist the full content, then appends a reference to
the truncated message:

```
[full output: tmp/tool-results/YYYY-MM-DD/{tool_name}_{hash}.out]
```

The path is relative to the workspace root so the agent can read it back with
